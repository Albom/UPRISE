...
Print
Color 7
Print "Integrate - программа накопления по времени S-файлов системы К3"
...
'/
	...
	For h = 0 To as_file_str.nh - 1 ' по высоте
		...
		For tau = 0 To 18 ' по задержке
			...
			For i = 0 To tNak-1
				...
				as_file_str.acf[h].rs(tau) += seans_str_out(h).datSin(tau)[t+i]
			Next i
	...
	For h = 12 To as_file_str.nh - 1 ' по высоте
