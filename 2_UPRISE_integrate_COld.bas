...
Print
Color 7
Print "Integrate - программа накопления по времени C-файлов системы К1"
...
Print #file, "Рассчитывать дисперсию? (0 - нет, 1 - да): "; isVar
'Print #file, "Нормировать АКФ? (0 - нет, 1 - да): "; isR
Close #file
...
'/
			...
			seans_str_out(h).dat1(0)[i] =  CDbl(seans_str_in[t].seans.dat1(h, 0))
			seans_str_out(h).dat2(0)[i] =  CDbl(seans_str_in[t].seans.dat2(h, 0))

			For tau = 1 To 6 ' по задержке
				seans_str_out(h).dat1(tau+6)[i] =  CDbl(seans_str_in[t].seans.dat1(h, tau))
				seans_str_out(h).dat2(tau+6)[i] =  CDbl(seans_str_in[t].seans.dat2(h, tau))
			Next tau

			' мощность по короткому импульсу
			seans_str_out(h).pShort[i] = 0
...
'/
	...
	For tau = 0 To 18 ' по задержке
		...
		For h = 250 To 300 ' по высоте
			as_file_str.rnc(tau) += as_file_str.acf[h].rc(tau)
		Next h
		as_file_str.rnc(tau) /= 50
	Next tau
