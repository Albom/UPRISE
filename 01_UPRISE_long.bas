...
file = FreeFile()
Open "Long.txt" For Output As #file
	...
	For i = 0 To lst_len-1
		...
		If (isM <> 0) Then
			...
			For h As Integer = 0 To 679
				seans_str(seans_loaded).p1(h) = seans.dat1(h, 0) - CDbl(seans.dat01(h))*CDbl(seans.dat01(h))/1463.0
				seans_str(seans_loaded).p2(h) = seans.dat3(h, 0) - CDbl(seans.dat02(h))*CDbl(seans.dat02(h))/1463.0
