...
Print
Input "Введите количество точек: ", num_points
...
file = FreeFile
Open SEANS_DIR_OUT + DirectoryOutput+"/step5/"+"T.txt" For Output As #file
Close #file
...
For h = Hmin To Hmax Step Hstep ' по высоте
	...
	file = FreeFile()
	Open SEANS_DIR_OUT + DirectoryOutput+"/step5/V."+ Str(CInt(Hkm(h))) +".txt" For Output As #file
