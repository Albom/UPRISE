...
ButtonGadget(BTN_SPLINE,    10, 170, 220, 28, "2. Заполнение пропусков")
ButtonGadget(BTN_INTEGRATE, 10, 200, 220, 28, "3. Временное усреднение")
ButtonGadget(BTN_APPROX,    10, 230, 220, 28, "4. Высотная коррекция")
ButtonGadget(BTN_ESTIMATE,  10, 260, 220, 28, "5. Оценка параметров")
		...
		Case EventGadget
			Select Case EventNumber
				...
				Case BTN_SAVE
					...
					Var GagetText = TextGadget(TEXT_MESSAGE,10,10,540,30,"Укажите формат экспорта")
				...
				'/
					...
					directory = ShellFolder("Укажите каталог с результатами (step4 или step5)", GetCurentDir()+"\out", BIF_USENEWUI Or BIF_NONEWFOLDERBUTTON)
					...
					If Len(directory) > 0 Then
						...
						If result = EXPORT_OK Then
							MessBox("UPRISE", "Экспорт успешно завершён", MB_ICONINFORMATION)
						Else 
