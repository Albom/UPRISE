...
Dim Shared As Integer libraries_min = 100
...
file = FreeFile()
Open "hardware/filter_COld.dat" For Input As #file
If Err <> 0 Then
...
For h = hMax To hMin Step -hStep ' по высоте
	...
	inverse_problem_v1(h, Config_step_ox_3, Config_step_te_3, Config_step_ti_3)
...
Sub save_all()
	...
	For h = 0 To nH-1
		...
		file = FreeFile()
		Open SEANS_DIR_OUT + DirectoryOutput+"/step3"+"/O.-1."+ Str(h_array(h)) +".txt" For Input As #file
		For t = 0 To nT-1
	...
	file = FreeFile()
	Open SEANS_DIR_OUT + DirectoryOutput+"/step3"+"/O.txt" For Output As #file
